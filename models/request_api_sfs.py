...
def post_actualiza(_IP_API):
    ...
    print('Actualiza pantalla')
   

# Genera y firma xml
def post_genera_xml(num_ruc, tip_docu, num_docu, _IP_API):

    """Actualiza bandeja antes de firmar xml"""
    post_actualiza(_IP_API)
...
# Envia xml a sunat
def post_envia_xml(num_ruc, tip_docu, num_docu, _IP_API):
    ...
    response = requests.request("POST", _IP_API+_ENVIA_SUNAT, headers=headers, data=payload)
...
# Genera pdf en /REPO
def post_report_sfs(num_ruc, tip_docu, num_docu, _IP_API):
